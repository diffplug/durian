...
public class MapConstraintsTest extends TestCase {
	...
	public void testConstrainedTypePreservingSortedSet() {
		...
		SortedSetMultimap<String, Integer> delegate = TreeMultimap.create(Ordering.<String> natural(), comparator);
		SortedSetMultimap<String, Integer> multimap = MapConstraints.constrainedSortedSetMultimap(delegate,
				TEST_CONSTRAINT);
		multimap.put("foo", 1);
		Map.Entry<String, Collection<Integer>> entry = multimap.asMap().entrySet().iterator().next();
	...
	public void testMultimapAsMapEntriesToArray() {
		...
		multimap.put("foo", 1);
		@SuppressWarnings("unchecked")
		Map.Entry<String, Collection<Integer>> entry = (Map.Entry<String, Collection<Integer>>) constrained.asMap().entrySet().toArray()[0];
		try {
	...
	public void testMultimapAsMapEntriesContainsNefariousEntry() {
		...
		Multimap<String, Integer> constrained = MapConstraints.constrainedMultimap(multimap, TEST_CONSTRAINT);
		multimap.put("foo", 1);
		Map.Entry<String, ? extends Collection<Integer>> nefariousEntry = nefariousMapEntry(TEST_KEY, Collections.singleton(TEST_VALUE));
